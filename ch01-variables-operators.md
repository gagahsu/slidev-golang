...
- **GoShop 專案實作** — 第 1 步：用變數、常數試算一筆訂單
- **章節總結**
...
上一章我們只建立了專案、印出歡迎訊息。今天學了變數、常數、算符和指標，剛好可以讓 GoShop 算出第一筆訂單的金額。
-->
...
顧客是**金卡會員**，買了 3 包「衣索比亞咖啡豆」（單價 450 元，庫存 20 包）

1. 用**常數**宣告商店名稱 `shopName` 與免運門檻 `freeShipping`（1000 元）
2. 用 **`iota`** 宣告會員等級 `Normal`、`Silver`、`Gold`，每升一級多折 5%
3. 算出小計、折扣、總計，並用比較與邏輯算符判斷是否免運
4. 寫一個函式 `sell(stock *int, qty int)`，用**指標**扣掉庫存
...
<!--
情境是這樣：一位金卡會員買了 3 包咖啡豆，我們要算出他要付多少錢。

會員等級用 iota 來宣告：一般會員是 0、銀卡是 1、金卡是 2。這裡有個小巧思：折扣百分比剛好可以用「等級乘以 5」算出來，金卡就是 10% 的折扣。

最後一個要求是寫一個 sell 函式扣庫存。如果參數用 int 而不是 *int，函式裡改的只是一份複本，外面的庫存不會變，這正是今天「值 vs. 指標」的重點。

大家先自己試試看，目標是印出跟下面一模一樣的結果。
-->
...
第一組 const 放商店的設定：店名和免運門檻，這些值在程式執行期間不會改變，所以用常數。

第二組 const 用 iota 列舉會員等級，Normal 是 0、Silver 是 1、Gold 是 2。

sell 函式的參數 stock 是 *int，也就是指向 int 的指標。函式裡用 *stock 解參考，把指到的那個變數減掉 qty，這樣呼叫端的庫存才會真的被扣掉。
-->
...
折扣的算式是 subtotal 乘以 level 再乘以 5、除以 100。這裡要注意運算順序：整數除法會捨去小數，所以要「先乘後除」，如果寫成 level * 5 / 100 先算，結果會是 0。

免運的判斷結合了比較算符 >= 和邏輯算符 &&，結果是一個 bool。

呼叫 sell 的時候傳入 &stock，也就是 stock 的位址。執行後庫存從 20 變成 17。

中間的 // ... 表示省略了幾行 Println，完整程式碼在 goshop/ch01。
-->
...
- **GoShop**：用常數、`iota` 與算符試算訂單金額，用指標扣庫存
...
最後我們在 GoShop 專案裡用上了今天的所有東西：常數存放不會變的設定，iota 列舉會員等級，算符計算金額，指標讓函式可以直接扣掉庫存。
