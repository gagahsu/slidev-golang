...
func placeOrder(ctx context.Context, st store.Store, opt options) error {
	...
	if opt.webhook != "" {
		...
		err := webhook.Send(ctx, client, opt.webhook, webhook.Event{Type: "order.paid", Data: o})
		if err != nil {
			slog.Warn("通知失敗", "err", err) // 訂單已成立，通知失敗只記錄下來
		}
	}
