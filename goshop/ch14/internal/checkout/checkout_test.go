...
func TestCheckout(t *testing.T) {
	...
	o, err := svc.Checkout(t.Context(), cart)
	if err != nil {
	...
	if p, _ := svc.Store.Product(t.Context(), "A"); p.Stock != 8 {
...
func TestCheckoutErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svc := newService()
			_, err := svc.Checkout(t.Context(), Cart{Items: tt.items})
			if !errors.Is(err, tt.want) {
				t.Fatalf("err = %v，want %v", err, tt.want)
			}
			// 結帳失敗時，庫存不能被扣掉
			if p, _ := svc.Store.Product(t.Context(), "A"); p.Stock != 10 {
...
func TestPay(t *testing.T) {
	svc := newService()
	o, _ := svc.Checkout(t.Context(), Cart{Items: []Item{{"A", 1}}})

	wallet := &payment.Wallet{Balance: 100}
	err := svc.Pay(t.Context(), o.ID, wallet, payment.CashOnDelivery{})
	if err != nil {
		t.Fatalf("Pay() error = %v", err)
	}
	got, _ := svc.Store.Order(t.Context(), o.ID)
	if got.Status != shop.Paid || got.PaidBy != "貨到付款" {
		t.Errorf("訂單 = %v／%s，want 已付款／貨到付款", got.Status, got.PaidBy)
	}
	if err := svc.Pay(t.Context(), o.ID, wallet); !errors.Is(err, shop.ErrPaid) {
...
func TestCoupon(t *testing.T) {
	...
	o, err := svc.Checkout(t.Context(), Cart{Items: []Item{{"A", 2}}, Coupon: "FALL15"})
	if err != nil {
		t.Fatalf("Checkout() error = %v", err)
	}
	...
	for _, code := range []string{"SUMMER", "NOPE"} {
		_, err := svc.Checkout(t.Context(), Cart{Items: []Item{{"A", 1}}, Coupon: code})
