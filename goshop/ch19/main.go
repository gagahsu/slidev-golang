...
type options struct {
	...
	version   bool   // 印出版本
	tlsCert   string // TLS 憑證檔
	tlsKey    string // TLS 私鑰檔
	hashPw    bool   // 產生管理員密碼的 bcrypt 雜湊
}
...
func main() {
	...
	flag.StringVar(&opt.tlsCert, "tls-cert", "", "TLS 憑證檔（cert.pem），指定後改用 HTTPS")
	flag.StringVar(&opt.tlsKey, "tls-key", "", "TLS 私鑰檔（key.pem）")
	flag.BoolVar(&opt.hashPw, "hash-password", false, "從標準輸入讀取密碼，印出 bcrypt 雜湊")
	flag.Parse()
...
// printHash 讀取一行密碼，印出可以設定到 GOSHOP_ADMIN_HASH 的 bcrypt 雜湊
func printHash() error {
	fmt.Fprint(os.Stderr, "請輸入管理員密碼：")
	sc := bufio.NewScanner(os.Stdin)
	if !sc.Scan() || len(sc.Text()) < 8 {
		return errors.New("密碼至少需要 8 個字元")
	}
	hash, err := auth.HashPassword(sc.Text())
	if err != nil {
		return err
	}
	fmt.Println(hash)
...
// newAuth 從環境變數讀取管理員密碼雜湊與簽章金鑰
func newAuth() (*auth.Auth, error) {
	hash := os.Getenv("GOSHOP_ADMIN_HASH")
	if hash == "" {
		return nil, errors.New("請先用 -hash-password 產生雜湊，設定到 GOSHOP_ADMIN_HASH")
	}
	secret := []byte(os.Getenv("GOSHOP_SECRET"))
	if len(secret) < 32 {
		// 沒有設定金鑰：產生隨機金鑰，重新啟動後大家都要重新登入
		slog.Warn("GOSHOP_SECRET 未設定或少於 32 個字元，改用隨機金鑰")
		secret = []byte(rand.Text() + rand.Text())
	}
	return &auth.Auth{PasswordHash: []byte(hash), Secret: secret, TTL: 8 * time.Hour}, nil
}

// serve 啟動 HTTP(S) 伺服器，收到 Ctrl+C 或 SIGTERM 時優雅關閉
func serve(svc *checkout.Service, opt options) error {
	a, err := newAuth()
	if err != nil {
		return err
	}
	...
	app := &web.Server{Store: svc.Store, Checkout: svc, Auth: a}
	srv := &http.Server{
		Addr:              opt.addr,
		Handler:           app.Handler(),
	...
	slog.Info("伺服器啟動", "addr", opt.addr, "https", opt.tlsCert != "",
		"version", version)

	select {
	case err := <-errCh: // 例如連接埠已被佔用、憑證檔不存在
		return err
	case <-ctx.Done():
	}
